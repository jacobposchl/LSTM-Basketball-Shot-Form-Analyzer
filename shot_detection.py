...
class ShotDetector:
        ...
        if self.state == ShotState.WAITING_FOR_STABILITY:
            ...
            if len(self.velocity_history) == CONSECUTIVE_FRAMES:
                ...
                if all(v > VELOCITY_THRESHOLD for v in self.velocity_history):
                    ...
                    # If velocities do not exceed the threshold, continue in READY_TO_DETECT_SHOT state
                    self.logger.debug(
                        "Velocity history does not exceed VELOCITY_THRESHOLD. Continuing READY_TO_DETECT_SHOT."
                    )
            ...
            if not self.is_ball_close:
                ...
                if self.distance:
                    ...
                    if self.distance > DISTANCE_THRESHOLD and shot_duration <= TIME_THRESHOLD:
                        ...
                        # If conditions for shot end are not met, continue in SHOT_IN_PROGRESS state
                        self.logger.debug("Shot end conditions not met. Continuing SHOT_IN_PROGRESS.")
                else:
                ...
                if shot_duration > TIME_THRESHOLD:
                    ...
                    # If time threshold not yet exceeded, continue in SHOT_IN_PROGRESS state
                    self.logger.debug(
                        "Ball still close and TIME_THRESHOLD not yet exceeded. "
                        "Continuing SHOT_IN_PROGRESS."
                    )

        elif self.state == ShotState.COOLDOWN:
            # Decrement the cooldown counter each frame
            self.cooldown_counter -= 1
            self.logger.debug(f"Cooldown counter at {self.cooldown_counter}")
    ...
    def calculate_distance(self, wrist_pos, ball_pos):
        ...
        self.logger.debug(f"Calculated distance between wrist and ball: {distance:.2f}")
